...
Add `'WosJohn\SMS\SMSServiceProvider'` in your `app/config/app.php` configuration file within the `providers` array.

###### Laravel 5

Add `'WosJohn\SMS\SMSServiceProvider'` in your `config/app.php` configuration file within the `providers` array.
...
Add `'SMS' => 'WosJohn\SMS\Facades\SMS'` in your `app/config/app.php` configuration file within the `aliases` array.

###### Laravel 5

Add `'SMS' => 'WosJohn\SMS\Facades\SMS'` in your `config/app.php` configuration file within the `aliases` array.
...
###### Laravel 4
	...
	php artisan config:publish wosjohn/s-sms

This will copy the configuration files to your `app/config/wosjohn/s-sms` folder.
...
Simple SMS operates in much of the same way as the Laravel Mail service provider.  If you are familiar with this then SMS should feel like home.  The most basic way to send a SMS is to use the following:
	...
	SMS::send('s-sms::welcome', $data, function($sms) {
		$sms->to('+15555555555');
	});
...
The `queue` method queues a message to be sent later instead of sending the message instantly.  This allows for faster respond times for the consumer by offloading uncustomary processing time. Like `Laravel's Mail` system, queue also has `queueOn,` `later,` and `laterOn` methods.
	...
	SMS::queue('s-sms::welcome', $data, function($sms) {
		$sms->to('+15555555555');
	});
